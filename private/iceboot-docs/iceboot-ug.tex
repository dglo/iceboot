...
\cmd{disableCurrents}{}{result}{address of flag to set to disable printing
  of currents on the top of the screen}
\cmd{disableFB}{}{}{Power-down and disable the flasherboard}
...
\cmd{enableFB}{}{resettime configtime validtime status}{ Enable and power-up the
  ...
  configure the CPLD, in us; validtime is the time required for
  the clock to stabilize during power-up (in us); and resettime is the
  total time required for the CPLD to reset, including the CPLD configuration and
  clock stability periods (in us).}
...
\cmd{fb-cpld}{xsvfAddress xsvfLength}{status}{Program the flasher board
CPLD with the XSVF file at xsvfAddress with length xsvfLength, return 
status = 0 if all is ok, otherwise return error code.}
\example{ $>$ s" flasher.xsvf" find if fb-cpld endif}{Program the
flasher board CPLD with the XSVF file named flasher.xsvf in the flash 
filesystem}

\cmd{fbid}{}{address length}{Get address and length of flasherboard ID string}
\example{ $>$ crlf fbid type type \\ ed00000a3daa5a01}{Print the
flasherboard ID string}
...
\cmd{hvid}{}{address length}{Get address and length of HV base ID string}
\example{ $>$ crlf hvid type type \\ 00000a4478e8}{Print the
HV base ID string}
...
\cmd{setFBmux}{channel}{}{ Select which LED current is muxed back to the
  ...
  driver pulse. }
\cmd{setFBrate}{rate}{}{ Set the rate (in Hz) for the flasherboard pulses.
 Rates supported are 610, 305, 153, 76, 38, 19, 10, 5, 2, and 1 Hz. }
...
\cmd{writeActiveBaseDAC}{value}{}{write value to PMT active base DAC}
...
\cmd{writePassiveBaseDAC}{value}{}{write value to PMT passive base DAC}
